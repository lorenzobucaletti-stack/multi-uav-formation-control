%% Part II - Formation Control of Nonlinear Planar Mobile Robots 
% Con Feedforward
clear; clc; close all;
...
% Trajectory selector
p.traj_type    = 'sinusoidal';   % linear| circular | sinusoidal
...
p.H = [0, 1, 0.5; 
       0, 0, sqrt(3)/2]; % Formato 2x3: [h1, h2, h3]
...
if p.SHOW_FREQUENCY_ANALYSIS
    ...
    yline(ax3, 6, 'k--', 'M_s = 6 dB', 'LineWidth', 1.2, ...
...
%% 4. POST-PROCESSING ED ESTRAZIONE DATI
...
%% PLOT
%RICALCOLO DEGLI INGRESSI FISICI
Nt = length(t);
...
for k = 1:Nt
    ...
    % Chiamiamo la dinamica per ottenere le accelerazioni effettive 
...
% Sforzo di Controllo (Ingressi U)
...
function [r, dr, ddr] = get_reference(t, p)
    ...
    elseif strcmp(p.traj_type, 'circular')
        ...
        ddr = [-p.R_c*p.omega_c^2*cos(p.omega_c*t); -p.R_c*p.omega_c^2*sin(p.omega_c*t)];
    else % sinusoidal
        r   = [p.v_sin*t; p.A_sin*sin(p.omega_sin*t)];
        dr  = [p.v_sin; p.A_sin*p.omega_sin*cos(p.omega_sin*t)];
        ddr = [0; -p.A_sin*(p.omega_sin^2)*sin(p.omega_sin*t)];
    end
end
...
function dX = formation_dynamics(t, X, p)
    ...
    % Sensori (Misure con rumore e disturbo opzionale)
    ...
    [r_t, dr_t, ddr_t] = get_reference(t, p);
    ...
    % Inizializzo derivate del filtro
    dz_x = zeros(3,1);
    dz_y = zeros(3,1);
    ...
    if p.USE_LEAD
        ...
        % Aggiunta del Feedforward (ddr_t) per annullare l'errore di tracciamento
        nu_x = p.Kp * epsilon_p_x + z_x + ddr_t(1);
        nu_y = p.Kp * epsilon_p_y + z_y + ddr_t(2);
    else
        % Anche per il PD Puro
        nu_x = p.Kp * epsilon_p_x + p.Kd * d_epsilon_p_x + ddr_t(1);
        nu_y = p.Kp * epsilon_p_y + p.Kd * d_epsilon_p_y + ddr_t(2);
    end
